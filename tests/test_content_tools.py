...
@pytest.mark.asyncio
async def test_add_heading(temp_docx_file):
...
@pytest.mark.asyncio
async def test_add_paragraph(temp_docx_file):
    """Test adding a paragraph to a document."""
    result = await content_tools.add_paragraph(temp_docx_file, "This is a new paragraph.")
    assert result["status"] == "success"
    assert "Paragraph added" in result["message"]

@pytest.mark.asyncio
async def test_add_table(temp_docx_file):
...
@pytest.mark.asyncio
async def test_add_picture_with_patch(temp_docx_file, tmp_path):
...
@pytest.mark.asyncio
async def test_add_page_break(temp_docx_file):
...
@pytest.mark.asyncio
async def test_add_table_of_contents(temp_docx_file):
...
@pytest.mark.asyncio
async def test_delete_paragraph(temp_docx_file):
    """Test deleting a paragraph from a document."""
    await content_tools.add_paragraph(temp_docx_file, "Paragraph to be deleted.")
    result = await content_tools.delete_paragraph(temp_docx_file, 1) # Delete the second paragraph
    assert  result["status"] == "success"
    assert "Paragraph at index 1 deleted" in result["message"]

@pytest.mark.asyncio
async def test_search_and_replace(temp_docx_file):
